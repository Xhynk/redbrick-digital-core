						...
						foreach( $api_object->reviews as $review ){
							if( $review->status == 'publish' ){ ?>
								<?php
									...
									$_bad		= str_repeat( '★', 5 - $magic );
									$_good		= str_repeat( '★', $magic );
									...
									$content	= "<p class='_content'>$_stars . $_content . $_ellipses . $_more</p>";
									$reviewbody	= "<span itemprop='reviewBody'>$content</span>";
									...
									$meta_name	= ( $hide_reviewer == true ) ? "<span itemprop='author' itemscope itemtype='http://schema.org/Person'><meta itemprop='name' content='Alexander D.' /></span>"
